...
enum class TipoShift { LSL, LSR, ASR, ROR };

// Define os modos de endereçamento para LDR/STR.
enum class ModoIndexacao { PreIndexado, PosIndexado };

/**
 * @brief Estruturas de dados para passar informações entre os módulos.
 */

// Estrutura para encapsular os detalhes de um operando do tipo registrador com
// shift.
...
// A estrutura principal que representa uma instrução completamente
// decodificada. O Decodificador preenche esta struct, e o Executor a utiliza
// para trabalhar.
struct Instrucao {
  ...
  uint8_t rn = 0,
          rd = 0; // Registradores principais (primeiro operando e destino)
