...
CPU::CPU(size_t memoria_bytes) : cpsr(0), memoria(memoria_bytes) {
  // Zera todos os registradores
  r.fill(0);
}

void CPU::carregarPrograma(const std::string &nome_arquivo,
                           uint32_t endereco_inicial) {
  memoria.carregarDeArquivo(nome_arquivo, endereco_inicial);
  r[15] =
      endereco_inicial; // Define o Program Counter para o início do programa
}
...
void CPU::imprimirEstado() {
  ...
  // Imprime o CPSR
  std::cout << "\nCPSR: 0x" << std::hex << std::setfill('0') << std::setw(8)
            << cpsr << std::dec << "\n";
