...
Instrucao decodificarProcessamentoDeDados(uint32_t instr_binaria) {
  ...
  if (eh_imediato) {
    ...
    instr.valor_imediato =
        (imediato_8bits >> rot) | (imediato_8bits << (32 - rot));
  } else {
    instr.operando_shift.eh_valido = true;
    instr.operando_shift.reg_base = instr_binaria & 0b1111;
    instr.operando_shift.tipo =
        static_cast<TipoShift>((instr_binaria >> 5) & 0b11);
...
Instrucao decodificarAcessoMemoria(uint32_t instr_binaria) {
  ...
  instr.modo_idx =
      eh_pre_idx ? ModoIndexacao::PreIndexado : ModoIndexacao::PosIndexado;
...
Instrucao decodificar(uint32_t instr_binaria) {
  ...
  if (type == 0b000 || type == 0b001) {
    return Decoder::decodificarProcessamentoDeDados(instr_binaria);
  } else if (type == 0b010 || type == 0b011) {
    return Decoder::decodificarAcessoMemoria(instr_binaria);
  } else if (type == 0b101) {
    return Decoder::decodificarSalto(instr_binaria);
  }
